...
| A7 | NFR-2 baseline: log cloud/local/memory split on ~50 real multi-step tasks | NFR-2 | 🔧 | **Substantially answered; cloud-share $ still pending.** Corpus `benchmarks/multistep_tasks.py` (59 tasks, 31 verifiers); runner `benchmarks/a7_baseline.py` (local + `--cloud`). Reports: `results/a7_baseline_full.md` (local, 53 tasks) + `results/a7_cloud_full.md` (cloud, 59). **Findings (54 valid cloud tasks):** correctness **21/29 (72%)**, hard **8/14**; tier mix nearly flat across difficulty (LOCAL 100/92/90%) → step difficulty **entangled**, not separable; **6 confident-but-wrong** (m09, m11, x01, x04, x05, x06) → **logprob-only routing is miscalibrated → GSA + verification earn their keep.** Bedrock tool-calling now works (PR #82), so escalation runs — but the AWS token expired mid-run, so the **cloud/local/memory $ share is still unreliable**; re-run with a fresh token to finalize. |
